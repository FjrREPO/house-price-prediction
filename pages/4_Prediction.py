...
with st.form("test_form"):
    st.write(
        "Change these values and click 'Test' to see how they affect the prediction:"
    )
    test_bathroom = st.number_input(
        "Test Bathroom Value",
        min_value=1,
        max_value=10,
        value=input_data["bathroom"] if "last_prediction" in st.session_state else 2,
    )
    test_carport = st.number_input(
        "Test Carport Value",
        min_value=0,
        max_value=5,
        value=input_data["carport"] if "last_prediction" in st.session_state else 1,
    )
    ...
    if test_button and "last_prediction" in st.session_state:
        ...
        test_data["price"] = (
            1500000 * test_data["LT"]
            + 5000000 * test_data["bedroom"]
            + 100000000 * test_bathroom
            + 75000000 * test_carport
        )

        bathroom_factor = 10000000 * test_bathroom
        carport_factor = 6000000 * test_carport

        test_data["price_per_m2_land"] = (
            (test_data["price"] + bathroom_factor) / test_data["LT"]
            if test_data["LT"] > 0
            else 0
        )
        test_data["price_per_m2_building"] = (
            (test_data["price"] + carport_factor) / test_data["LB"]
            if test_data["LB"] > 0
            else 0
        )

        test_data["bathroom_impact"] = test_bathroom * 100
        test_data["carport_impact"] = test_carport * 100
        ...
        if models["rf"] is not None:
            try:
                ...
                if "predicted_price" in st.session_state:
                    ...
                    diff_pct = (
                        (diff / original_price) * 100 if original_price > 0 else 0
                    )

                    st.write(
                        f"Difference from original: {format_price(diff)} ({diff_pct:.2f}%)"
                    )

                    if abs(diff) < 1000:
                        st.warning(
                            "The model is not sensitive to these features - this indicates they have very low importance in the model"
                        )
            except Exception as e:
